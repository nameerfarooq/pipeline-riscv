...
"ALU2 Test "in {
test ( new ALU2())/*.withAnnotations(Seq(VerilatorBackendAnnotation))*/{c=>
// ALU operations
...
case ALU_SLT =>  if(src_a.toInt < src_b.toInt) 1 else 0   //( src_a < src_b ).asInstanceOf[Int]
case ALU_SLL =>  src_a << ( src_b & 0x1F )
case ALU_SLTU =>  if(src_a < src_b) 1 else 0 //( src_a < src_b ).asInstanceOf[Int]
...
val result1 : BigInt = if( result < 0)( BigInt (0xFFFFFFFFL ) + result +1) & 0xFFFFFFFFL
